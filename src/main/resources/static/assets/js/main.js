...
(function () {
  ...
  if (typeof i18next !== 'undefined' && typeof i18NextHttpBackend !== 'undefined') {
    // --- BEGIN Language Persistence ---
    const storedLang = localStorage.getItem('userLanguage');
    const defaultLang = 'en'; // Default language if nothing is stored
    let initialLang = storedLang || defaultLang;

    // If no language was stored, save the default one
    if (!storedLang) {
      localStorage.setItem('userLanguage', defaultLang);
    }
    // --- END Language Persistence ---

    i18next
      .use(i18NextHttpBackend)
      .init({
        lng: initialLang, // Use stored or default language
        debug: false,
        fallbackLng: 'en',
        backend: {
      ...
      .then(function (t) {
        // localize () будет вызван в рамках обработчика смены языка
        // Чтобы убедиться, что правильный язык выбирается визуально после init.
        // нам все еще нужно обновить пользовательский интерфейс при начальной нагрузке, если
        // хранящий язык отличается от состояния HTML по умолчанию.
        localize(); // Держите это, чтобы применять переводы на нагрузку
      });
  }
  ...
  if (languageDropdown.length) {
    ...
    for (let i = 0; i < dropdownItems.length; i++) {
      dropdownItems[i].addEventListener('click', function () {
        ...
        // --- BEGIN Language Persistence ---
        localStorage.setItem('userLanguage', currentLanguage); // Save selected language
        // --- END Language Persistence ---
