...
// Убедимся, что все необходимые компоненты существуют
  if (typeof i18next !== 'undefined' && typeof i18NextHttpBackend !== 'undefined') {

    // Инициализация библиотеки i18next
    i18next
        .use(i18NextHttpBackend)
        .init({
          // Определяем язык по умолчанию из настроек темы, либо 'en'
          lng: window.templateCustomizer ? window.templateCustomizer.settings.lang : 'en',
          debug: false, // Отключаем отладочные логи для рабочей версии
          fallbackLng: 'en', // Язык, который будет использоваться, если перевод не найден
          backend: {
            // Путь к файлам с переводами. {{lng}} будет заменен на код языка (en, ru, uk)
            loadPath: assetsPath + 'json/locales/{{lng}}.json'
          },
          returnObjects: true
        })
        .then(function (t) {
          // После успешной инициализации, сразу же переводим страницу
          localize();
        });
  }

// Находим выпадающий список языков
  ...
  if (languageDropdown) {
    ...
    // Вешаем обработчик клика на каждый элемент списка
    dropdownItems.forEach(item => {
      item.addEventListener('click', function () {
        ...
        // Используем i18next для смены языка
        i18next.changeLanguage(currentLanguage, (err, t) => {
          if (err) {
            return console.error('Ошибка при смене языка в i18next:', err);
          }

          // ВАЖНО: Мы НЕ вызываем window.templateCustomizer.setLang() здесь,
          // так как это конфликтовало с i18next и вызывало ошибку.
          // i18next сам управляет состоянием языка.

          // Вызываем функцию для смены направления текста (LTR/RTL), если нужно
          directionChange(textDirection);
          // Вызываем нашу главную функцию для обновления текста на странице
          localize();
          // Синхронизируем другие опции темы, если это необходимо
  ...
  /**
   * Функция для смены направления текста (LTR/RTL).
   * Эта логика относится к теме и не конфликтует с i18next.
   * @param {string} textDirection Направление текста 'ltr' или 'rtl'.
   */
   ...
   * Находит все элементы с атрибутом [data-i18n] и заменяет их содержимое на перевод.
   * Также управляет активным состоянием в выпадающем списке языков.
   */
  function localize() {
    // Находим все элементы, которые нужно перевести
    const i18nList = document.querySelectorAll('[data-i18n]');

    // Обновляем класс 'active' в выпадающем меню
    // Сначала убираем класс у всех
    document.querySelectorAll('.dropdown-language .dropdown-item').forEach(el => el.classList.remove('active'));
    // Затем добавляем класс текущему выбранному языку
    const currentLanguageEle = document.querySelector(`.dropdown-item[data-language="${i18next.language}"]`);
    if (currentLanguageEle) {
      currentLanguageEle.classList.add('active');
    }

    // Проходим по каждому элементу и вставляем перевод
    i18nList.forEach(function (item) {
      // Получаем ключ перевода из атрибута data-i18n
      const key = item.dataset.i18n;
      // Получаем сам перевод из i18next
