...
class CommercialPropertiesFileMapperTest {

    @Mock
    private FileService fileService;

    private CommercialPropertiesFileMapper mapper;

    @BeforeEach
    void setUp() {
        mapper = Mappers.getMapper(CommercialPropertiesFileMapper.class);
    ...
    void toEntity_happyPath() throws IOException {
        ...
        CommercialPropertiesFileRequest req = new CommercialPropertiesFileRequest();
        req.setId(100L);
        req.setName("Doc");
        req.setPath(file);

        CommercialPropertiesFile entity = mapper.toEntity(req);
    ...
    @Test
    void toResponse() {
        CommercialPropertiesFile entity = CommercialPropertiesFile.builder()
                .id(200L)
                .name("Resp")
                .path("uploads/r.pdf")
                .build();

        CommercialPropertiesFileResponse resp = mapper.toResponse(entity);
    ...
    void partialUpdate_withFile() throws IOException {
        CommercialPropertiesFile entity = new CommercialPropertiesFile();
        entity.setId(300L);
        entity.setName("Old");
    ...
    void partialUpdateList_add_update_remove() {
        List<CommercialPropertiesFile> entities = new ArrayList<>();
        CommercialPropertiesFile existing = CommercialPropertiesFile.builder()
                .id(1L).name("E1").path("p1.pdf").build();
        entities.add(existing);

        CommercialPropertiesFileRequest upd = new CommercialPropertiesFileRequest();
        upd.setId(1L);
        upd.setName("E1-upd");

        CommercialPropertiesFileRequest add = new CommercialPropertiesFileRequest();
        add.setId(null);
        add.setName("E2");

        List<CommercialPropertiesFileRequest> requests = List.of(upd, add);

        mapper.partialUpdateList(requests, entities);

        assertEquals(2, entities.size());
        CommercialPropertiesFile e1 = entities.stream().filter(e -> Long.valueOf(1L).equals(e.getId())).findFirst().orElseThrow();
        assertEquals("E1-upd", e1.getName());
        CommercialPropertiesFile e2 = entities.stream().filter(e -> e.getId() == null).findFirst().orElseThrow();
